...
class SnakeEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.observation_space = spaces.Box(low=0, high=1,
                                        shape=(BOARD_SIZE,BOARD_SIZE, 2), dtype=np.int32)
    ...
    def move_snake(self, action, snake):
        ...
        elif np.array_equal(self._state[head_y][head_x+1],self.BB):
            if (action==0):
                kill_itself = True
            possible_random_choices.remove(0)
        ...
        elif np.array_equal(self._state[head_y+1][head_x],self.BB):
            if (action==1):
                kill_itself = True
            possible_random_choices.remove(1)
    ...
    def foodspawn_assist(self):
        coords = np.argwhere(((self._state == self.NB).all(axis=2)))
        if (len(coords)>0):
    ...
    def check_if_hit(self, head_x, head_y):
        if (head_x == self.BOARD_SIZE or head_y == self.BOARD_SIZE \
            ...
            or np.array_equal(self._state[head_y][head_x],self.BB)):
            return True
        else:
            return False
    ...
    def step(self, action):
        ...
        # Set Neck and Tail
        self._state[master_neck_y][master_neck_x] = self.BB
