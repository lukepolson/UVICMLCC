...
class SnakeEnv(py_environment.PyEnvironment):
    ...
    def reset_board(self):
        ...
        # Create Board
        self._state = (np.array([([[0, 0]]*self.BOARD_SIZE) for i in range(self.BOARD_SIZE)], dtype=np.int32),
                       np.array([100, head_x, head_y], dtype=np.int32))
    ...
    def __init__(self, FOOD_REWARD, STEP_REWARD, HEALTH_CUTOFF, KILL_STEP_REWARD,
        ...
        self._observation_spec = (array_spec.BoundedArraySpec(
            ...
            shape=(3,), dtype=np.int32, minimum=0, maximum=self.M, name='health'))
        self.reset_board()
    ...
    def _step(self, action):
        ...
        self._state[1][0] = self.master_snake.get_health()
        ...
        # If not then add the new head coord the the board
            ...
            #self._state[0][master_head_y][master_head_x] = self.BB
            self._state[1][1], self._state[1][2] = self.master_snake.get_head_coords()
