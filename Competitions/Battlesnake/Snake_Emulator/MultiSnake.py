...
class SnakeEnv(py_environment.PyEnvironment):
    ...
    def reset_board(self):
        ...
        self._state[master_init_y_coord[0]][master_init_x_coord[0]] = [0, self.M, self.M, 0, self.M]
        self._state[enemy_init_y_coord[0]][enemy_init_x_coord[0]] = [0, self.M, self.M, 0, -self.M]
    ...
    def enemy_board_reset(self):
        ...
        self._state = [([[0, 0, 0, 0, 0]]*self.BOARD_SIZE) for i in range(self.BOARD_SIZE)]
        # Put player snake back on the board
        ...
        for i, (x, y) in enumerate(zip(master_x_coords, master_y_coords)):
                if i == 0:
                    self._state[y][x] = [0, self.M, self.M, 0, self.M]
                else:
                    self._state[y][x] = [self.M,0,0,0,self.M]
        # New enemy snake
        self.ENEMY_COUNTER = 0
    ...
    def __init__(self, MAX_HEALTH,
        ...
        self._observation_spec = array_spec.BoundedArraySpec(
            shape=(BOARD_SIZE,BOARD_SIZE, 5), dtype=np.int32, minimum=-self.M, maximum=self.M, name='observation')
        self.reset_board()
    ...
    def get_enemy_obs(self):
        enemy_state = np.array(self._state)
        np.apply_along_axis(self.enemy_state_helper, 2, enemy_state)
        return tf.convert_to_tensor([enemy_state])
    ...
    def check_if_hit(self, head_x, head_y):
        if (head_x == self.BOARD_SIZE or head_y == self.BOARD_SIZE \
            ...
            return True
        elif(self._state[head_y][head_x][0] == self.M):
            return True   
        else:
            return False
    ...
    def _step(self, action):
        ...
        # Set Neck 
        self._state[master_neck_y][master_neck_x] = [self.M,0,0,0,self.M]
        self._state[enemy_neck_y][enemy_neck_x] = [self.M,0,0,0,-self.M]
        ...
        elif (self.check_if_hit(enemy_head_x, enemy_head_y)):
        ...
        # If not then add the new head coord the the board
            ...
            block_enemy = self._state[enemy_head_y][enemy_head_x]
            
            # Then update the block
            self._state[master_head_y][master_head_x] = [0,self.M,self.master_snake.get_health(),0,self.M]
            self._state[enemy_head_y][enemy_head_x] = [0,self.M,self.enemy_snake.get_health(),0,-self.M]
            ...
            if(block_master==[0,0,0,self.M,0]):
            ...
            # If out of health then die
            if self.enemy_snake.get_health() == 0:
                self.enemy_board_reset()
