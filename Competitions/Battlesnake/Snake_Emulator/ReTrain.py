...
initial_collect_policy = tf.compat.v2.saved_model.load('../DATA/policy_{}'.format(II))
...
if __name__ == "__main__":
    ...
    def train_agent(n_iterations):
        ...
        for iteration in range(n_iterations):
            ...
            if iteration % 10000 == 0:
                ...
                f = open("../DATA/progress_{}_v2.txt".format(II), "w")
    ...
    df.to_csv('../DATA/stats_{}_v2.txt'.format(II), index=False, mode="w")
    
    # Store Frames
    frames = get_vid_frames(agent.policy, "trained-agent")
    with open('../DATA/frames_{}_v2.pkl'.format(II), 'wb') as f:
        pickle.dump(frames, f)
        
    # Store Model
    my_policy = agent.policy
    saver = PolicySaver(my_policy, batch_size=None)
    saver.save('../DATA/policy_{}_v2'.format(II))