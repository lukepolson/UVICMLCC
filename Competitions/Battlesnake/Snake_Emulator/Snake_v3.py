...
class SnakeEnv(py_environment.PyEnvironment):
    ...
    def reset_board(self):
        ...
        for i, (x, y) in enumerate(zip(master_x_coords, master_y_coords)):
                if i == 0:
                    self._state[y][x] = [0, self.M, self.M, 0]
                else:
                    self._state[y][x] = [self.M, 0, 0, 0]
    ...
    def __init__(self, FOOD_REWARD, STEP_REWARD, HEALTH_CUTOFF, KILL_STEP_REWARD,
                 DEATH_REWARD, BOARD_SIZE, MAX_HEALTH, FOOD_SPAWN_MODE, FOOD_REWARD_MODE):
        ...
        self._observation_spec = array_spec.BoundedArraySpec(
            shape=(BOARD_SIZE,BOARD_SIZE, 4), dtype=np.int32, minimum=-1, maximum=self.M, name='observation')
        self.reset_board()
    ...
    def apply_action(self, action):
        ...
        # Action 0
        if (master_head_x+1 == self.BOARD_SIZE):
            if (action==0):
                kill_itself = True
            possible_random_choices.remove(0)
        elif (self._state[master_head_y][master_head_x+1] == [self.M, 0, 0, 0]):
            if (action==0):
                kill_itself = True
            possible_random_choices.remove(0)
        
        # Action 1
        if (master_head_y+1 == self.BOARD_SIZE):
            if (action==1):
                kill_itself = True
            possible_random_choices.remove(1)
        elif (self._state[master_head_y+1][master_head_x] == [self.M, 0, 0, 0]):
            if (action==1):
                kill_itself = True
            possible_random_choices.remove(1)
        
        # Action 2
        if (master_head_x-1 == -1):
            if (action==2):
                kill_itself = True
    ...
    def _step(self, action):
        ...
        if (master_head_x == self.BOARD_SIZE or master_head_y == self.BOARD_SIZE \
            ...
            or self._state[master_head_y][master_head_x] == [self.M, 0, 0, 0]):
        ...
        # Else perform snake step
            ...
            if not(self.master_snake.get_just_eaten()):
                self._state[master_tail_y][master_tail_x] = [0,0,0,0]
            self.master_snake.set_just_eaten(False)

            # Check if snake consumed food
            if(block==[0,0,0,self.M]):
