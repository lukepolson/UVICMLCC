...
from MultiSnake_v2 import Snake
from MultiSnake_v2 import SnakeEnv
...
params_df = pd.read_pickle('../Training_Hyperparameters/train_params_multi.pkl')
print(sys.argv[1])
II = int(sys.argv[1])
...
'''
# This is a workaround to get an initial enemy policy based on the environment specs.
env = SnakeEnv(FOOD_REWARD=0, STEP_REWARD=1, HEALTH_CUTOFF=0, KILL_STEP_REWARD=-10,
                 WIN_REWARD = 100, DEATH_REWARD=-100, BOARD_SIZE=int(board_size), MAX_HEALTH=100, FOOD_SPAWN_MODE=1,
                    FOOD_REWARD_MODE=1, ENEMY_POLICY=None)
tf_env = tf_py_environment.TFPyEnvironment(env)
initial_enemy_policy = RandomTFPolicy(tf_env.time_step_spec(),
                                        tf_env.action_spec())
'''
env = SnakeEnv(FOOD_REWARD=food_reward, STEP_REWARD=step_reward, HEALTH_CUTOFF = health_cutoff,
               DEATH_REWARD=death_reward, WIN_REWARD=win_reward, FOOD_SPAWN_MODE=food_spawn_mode,
               KILL_STEP_REWARD = kill_step_reward, FOOD_REWARD_MODE = food_reward_mode, BOARD_SIZE=int(board_size),
               MAX_HEALTH=100, ENEMY_POLICY=None, MAX_COUNTER=max_counter)
...
# Layers params are specified by local variables obtained from DataFrame
q_net = QNetwork(
...
if __name__ == "__main__":
    ...
    def train_agent(n_iterations):
        ...
        for iteration in range(n_iterations):
            ...
            if iteration % 10000 == 0:
                ...
                f = open("../DATA/Multi/progress_{}.txt".format(II), "w")
    ...
    df.to_csv('../DATA/Multi/stats_{}.txt'.format(II), index=False, mode="a")
    
    # Store Frames
    frames = get_vid_frames(agent.policy, "trained-agent")
    with open('../DATA/Multi/frames_{}.pkl'.format(II), 'wb') as f:
        pickle.dump(frames, f)
        
    # Store Model
    my_policy = agent.policy
    saver = PolicySaver(my_policy, batch_size=None)
    saver.save('../DATA/Multi/policy_{}'.format(II))
